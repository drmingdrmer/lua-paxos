...
class Http(object):
    ...
    def read_body(self, size):
        ...
        if size > self.content_len - self.has_read:
            size = self.content_len - self.has_read
    ...
    def _load_resp_headers(self):
        ...
        if self.chunked:
            ...
            if length is None:
                raise HeadersError('should have content-length header')
            elif not length.isdigit():
                raise HeadersError('content-length header value error')
            else:
                self.content_len = int(length)
